...
const RoleTable = () => {
  ...
  const handleAddOrEdit = async (values) => {
    if (editingRole) {
      await axios.put(`http://127.0.0.1:8000/member/roles/${editingRole.id}/`, values);
      message.success('Role updated');
    } else {
      await axios.post('http://127.0.0.1:8000/member/roles/', values);
  ...
  const handleDelete = async (id) => {
    await axios.delete(`http://127.0.0.1:8000/member/roles/${id}/`);
